...
def ajusta_parametro(classifier_path, dataset_path, valor_parametro):
    """Treina um classificador Eigenfaces com um parâmetro específico e retorna o classificador."""
    ...
    print("Treinando novo classificador Eigenfaces...")
    try:
        eigen_classifier = cv2.face.EigenFaceRecognizer_create(num_components=valor_parametro)
        eigen_classifier.train(faces, ids)
        eigen_classifier.write(classifier_path)
        print(f"Classificador treinado e salvo em '{classifier_path}'.")
        return eigen_classifier
    except cv2.error as e:
...
if __name__ == "__main__":
    ...
    CLASSIFIER_PATH = "eigen_classifier.yml"
    
    # Carrega o modelo SSD
    network = cv2.dnn.readNetFromCaffe(MODEL_PROTO, MODEL_WEIGHTS)
    
    # Valor do parâmetro de calibragem do EigenFaces
    VALOR_PARAMETRO = 40
